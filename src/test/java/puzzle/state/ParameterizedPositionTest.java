...
class ParameterizedPositionTest {
    ...
    void getTarget(Position position) {
        assertPosition(position.row() - 1, position.col(), position.getTarget(Direction.UP));
        assertPosition(position.row(), position.col() + 1, position.getTarget(Direction.RIGHT));
        assertPosition(position.row() + 1, position.col(), position.getTarget(Direction.DOWN));
        assertPosition(position.row(), position.col() - 1, position.getTarget(Direction.LEFT));
    }
    ...
    @ParameterizedTest
    @MethodSource("positionProvider")
    void setTarget_up(Position position) {
    ...
    @ParameterizedTest
    @MethodSource("positionProvider")
    void setTarget_right(Position position) {
        var row = position.row();
        var col = position.col();
    ...
    @ParameterizedTest
    @MethodSource("positionProvider")
    void setTarget_down(Position position) {
        var row = position.row();
        var col = position.col();
    ...
    @ParameterizedTest
    @MethodSource("positionProvider")
    void setTarget_left(Position position) {
        var row = position.row();
        var col = position.col();
